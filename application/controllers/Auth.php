...
class Auth extends CI_Controller
{
    ...
    private function _sendEmail($token, $type)
    {
        ...
        $config['smtp_user'] = 'dawang.wayns@gmail.com'; //email yg dirubah keamannnya
        $config['smtp_pass'] = 'spfawfqjjvebwjne'; //isikan password dr kelola akun gmail
        ...
        $this->email->from('dawangwahyunas@gmail.com', 'Web programing SMKN8Jember');
        $this->email->to($this->input->post('email'));

        if ($type == 'verify') {
            $this->email->subject('Account Verification');
            $this->email->message('Click this link to verify you account :
                <a href="' . base_url() . 'auth/veriify?email=' . $this->input->post('email') . 
